 ...
 * )
 */
class GnfacMediaArt extends SqlBase {
  /**
   * {@inheritdoc}
   */
    ...
    $row->setSourceProperty('teaser', $revision_data[0]['teaser']);
		...
		foreach( $uploads_results as $result ){
			$row->setSourceProperty('attached_fid', $result['fid']);
				...
				$images_query->join('image_attach', 'ia' , 'ia.iid = i.nid');  // but ia.nid is the nid of the advisory node ( or media_art node, or whatever.)
